...
main(int ac, const char* av[])
{
    ...
    lozzeg::CmdLineOptions opts {ac, av};
    ...
    lozzeg::rpccalls::login_opt daemon_rpc_login {};
    ...
    if (!lozzeg::get_blockchain_path(bc_path_opt, blockchain_path, nettype))
    {
    ...
    lozzeg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!lozzeg::init_blockchain(blockchain_path.string(),
    ...
    if (testnet && deamon_url == "http:://127.0.0.1:11112")
        deamon_url = "http:://127.0.0.1:28081";
    if (stagenet && deamon_url == "http:://127.0.0.1:11112")
    ...
    if (enable_emission_monitor == true)
    {
        ...
        lozzeg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        lozzeg::CurrentBlockchainStatus::nettype
                = nettype;
        lozzeg::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        lozzeg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        lozzeg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    lozzeg::MempoolStatus::blockchain_path
            = blockchain_path;
    lozzeg::MempoolStatus::nettype
            = nettype;
    lozzeg::MempoolStatus::deamon_url
            = deamon_url;
    lozzeg::MempoolStatus::login
            = daemon_rpc_login;
    lozzeg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);

    lozzeg::MempoolStatus::network_info initial_info;
    strcpy(initial_info.block_size_limit_str, "0.0");
    strcpy(initial_info.block_size_median_str, "0.0");
    lozzeg::MempoolStatus::current_network_info = initial_info;
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    lozzeg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    lozzeg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    lozzeg::page lozzblocks(&mcore,
                          core_storage,
                          deamon_url,
                          nettype,
                          enable_pusher,
    ...
    CROW_ROUTE(app, "/")
    ([&]() {
        return mylozz::htmlresponse(lozzblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return mylozz::htmlresponse(lozzblocks.index2(page_no));
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](size_t block_height) {
        return mylozz::htmlresponse(lozzblocks.show_block(block_height));
    });
    
    CROW_ROUTE(app, "/randomx/<uint>")
    ([&](size_t block_height) {
        return mylozz::htmlresponse(lozzblocks.show_randomx(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](string block_hash) {
        return mylozz::htmlresponse(
                lozzblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](string tx_hash) {
        return mylozz::htmlresponse(
                lozzblocks.show_tx(remove_bad_chars(tx_hash)));
    });
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash) {
            ...
            return mylozz::htmlresponse(
                lozzblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures, refresh_page));
        });
    }
    ...
    if (enable_as_hex)
    {
        ...
        ([&](string tx_hash) {
            return crow::response(
                    lozzblocks.show_tx_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/ringmembershex/<string>")
        ([&](string tx_hash) {
            return crow::response(
                    lozzblocks.show_ringmembers_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/blockhex/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    lozzblocks.show_block_hex(block_height, false));
        });

        CROW_ROUTE(app, "/blockhexcomplete/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    lozzblocks.show_block_hex(block_height, true));
        });

//        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
//        ([&](string tx_hash) {
//            return crow::response(
//              lozzblocks.show_ringmemberstx_hex(remove_bad_chars(tx_hash)));
//        });

        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return mylozz::jsonresponse {
                lozzblocks.show_ringmemberstx_jsonhex(
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash, uint16_t with_ring_signature) {
            bool refresh_page {true};
            return mylozz::htmlresponse(
                lozzblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signature, refresh_page));
        });
    }

    CROW_ROUTE(app, "/myoutputs").methods("POST"_method)
    ([&](const crow::request& req) -> mylozz::htmlresponse
     {

        map<std::string, std::string> post_body
                = lozzeg::parse_crow_post_data(req.body);

        if (post_body.count("lozz_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("lozz address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string lozz_address = remove_bad_chars(post_body["lozz_address"]);
        ...
        string response = lozzblocks.show_my_outputs(
                                         tx_hash, lozz_address,
                                         viewkey, raw_tx_data,
                                         domain);

        return mylozz::htmlresponse(std::move(response));
    });

    CROW_ROUTE(app, "/myoutputs/<string>/<string>/<string>")
    ([&](const crow::request& req, string tx_hash,
        string lozz_address, string viewkey)
     {

        string domain = get_domain(req);

        return mylozz::htmlresponse(lozzblocks.show_my_outputs(
                                         remove_bad_chars(tx_hash),
                                         remove_bad_chars(lozz_address),
        ...
        ([&](const crow::request& req) -> mylozz::htmlresponse 
         {
            ...
            if (post_body.count("lozzaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("lozz address, tx private key or "
            ...
            string lozz_address = remove_bad_chars(post_body["lozzaddress"]);
            ...
            return mylozz::htmlresponse(lozzblocks.show_prove(tx_hash,
                                        lozz_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain));
    ...
    ([&](const crow::request& req, string tx_hash,
         string lozz_address, string tx_prv_key) 
     {

        string domain = get_domain(req);

        return mylozz::htmlresponse(lozzblocks.show_prove(
                                    remove_bad_chars(tx_hash),
                                    remove_bad_chars(lozz_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&]() {
            return mylozz::htmlresponse(lozzblocks.show_rawtx());
        });
        ...
        ([&](const crow::request& req) -> mylozz::htmlresponse
         {
            ...
            if (action == "check")
                return mylozz::htmlresponse(
                        lozzblocks.show_checkrawtx(raw_tx_data, action));
            else if (action == "push")
                return mylozz::htmlresponse(
                        lozzblocks.show_pushrawtx(raw_tx_data, action));
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&]() {
            return mylozz::htmlresponse(lozzblocks.show_rawkeyimgs());
        });
        ...
        ([&](const crow::request& req) -> mylozz::htmlresponse
         {
            ...
            return mylozz::htmlresponse(
                    lozzblocks.show_checkrawkeyimgs(raw_data, viewkey));
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&]() {
            return mylozz::htmlresponse(lozzblocks.show_rawoutputkeys());
        });
        ...
        ([&](const crow::request& req) -> mylozz::htmlresponse
         {
            ...
            return mylozz::htmlresponse(
                    lozzblocks.show_checkcheckrawoutput(raw_data, viewkey));
        });
    }
    ...
    ([&]() {
        return mylozz::htmlresponse(lozzblocks.mempool(true));
    });
...
//    });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](string search_value) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&]() {
            ...
            mylozz::jsonresponse r{lozzblocks.json_networkinfo()};
        ...
        ([&]() {
            ...
            mylozz::jsonresponse r{lozzblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_outputs(
                    remove_bad_chars(tx_hash),
        ...
        ([&](const crow::request &req) {
            ...
            mylozz::jsonresponse r{lozzblocks.json_outputsblocks(
        ...
        ([&]() {
            ...
            mylozz::jsonresponse r{lozzblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            uint64_t page_no {0};
            bool refresh_page {true};
            return mylozz::htmlresponse(lozzblocks.index2(page_no, refresh_page));
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        lozzeg::CurrentBlockchainStatus::m_thread.interrupt();
        lozzeg::CurrentBlockchainStatus::m_thread.join();
    ...
    lozzeg::MempoolStatus::m_thread.interrupt();
    lozzeg::MempoolStatus::m_thread.join();
