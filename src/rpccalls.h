...
#ifndef CROWLOZZ_RPCCALLS_H
#define CROWLOZZ_RPCCALLS_H
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Lozzax deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Lozzax deamon at "
                 << deamon_url << endl;
            return false;
        }
