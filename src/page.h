...
#ifndef CROWLOZZ_PAGE_H
#define CROWLOZZ_PAGE_H
...
struct tx_details
{
    ...
    uint64_t lozz_inputs;
    uint64_t lozz_outputs;
    ...
    // public keys and lozz amount of outputs
    ...
    get_mstch_map() const
    {
        ...
        const double& lozz_amount = LOZZ_AMOUNT(fee);
        ...
        {
            double payed_for_kB = lozz_amount / tx_size;

            mixin_str        = std::to_string(mixin_no);
            fee_str          = fmt::format("{:0.6f}", lozz_amount);
            fee_short_str    = fmt::format("{:0.4f}", lozz_amount);
            fee_micro_str    = fmt::format("{:04.0f}" , lozz_amount * 1e6);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , lozz_amount_to_str(lozz_inputs , "{:0.6f}")},
                {"sum_outputs"       , lozz_amount_to_str(lozz_outputs, "{:0.6f}")},
                {"sum_inputs_short"  , lozz_amount_to_str(lozz_inputs , "{:0.3f}")},
                {"sum_outputs_short" , lozz_amount_to_str(lozz_outputs, "{:0.3f}")},
          ...
          mainnet_url {_mainnet_url}
{
    ...
    template_file["css_styles"]      = lozzeg::read(TMPL_CSS_STYLES);
    template_file["header"]          = lozzeg::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(lozzeg::read(TMPL_INDEX2));
    template_file["mempool"]         = lozzeg::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(lozzeg::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = lozzeg::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["block"]           = get_full_page(lozzeg::read(TMPL_BLOCK));
    template_file["randomx"]         = get_full_page(lozzeg::read(TMPL_RANDOMX));
    template_file["tx"]              = get_full_page(lozzeg::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(lozzeg::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(lozzeg::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(lozzeg::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(lozzeg::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(lozzeg::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(lozzeg::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(lozzeg::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(lozzeg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(lozzeg::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(lozzeg::read(TMPL_SEARCH_RESULTS));
    template_file["tx_details"]      = lozzeg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_table_header"] = lozzeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = lozzeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");
}
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , lozzeg::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    //double blk_size_median = lozzeg::calc_median(blk_sizes.begin(), blk_sizes.end());
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        string emission_coinbase = lozz_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = lozz_amount_to_str(current_values.fee, "{:0.3f}");
...
mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
{
    ...
    for (size_t i = 0; i < no_of_mempool_tx; ++i)
    {
        ...
        txs.push_back(mstch::map {
                ...
                {"lozz_inputs"      , mempool_tx.lozz_inputs_str},
                {"lozz_outputs"     , mempool_tx.lozz_outputs_str},
...
show_block(uint64_t _blk_height)
{
    ...
    string blk_timestamp = lozzeg::timestamp_to_str_gm(blk.timestamp);
    ...
    for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
    {
        ...
        //string mixin_times_scale = lozzeg::timestamps_time_scale(mixin_timestamps,
        //                                                        server_timestamp);
    ...
    context["sum_fees"]
            = lozzeg::lozz_amount_to_str(sum_fees, "{:0.6f}", false);

    // get lozz in the block reward
    context["blk_reward"]
            = lozzeg::lozz_amount_to_str(txd_coinbase.lozz_outputs - sum_fees, "{:0.6f}");
...
show_block(string _blk_hash)
{
    ...
    if (!lozzeg::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0, bool refresh_page = false)
{
    ...
    if (!lozzeg::parse_str_secret_key(tx_hash_str, tx_hash))
    {
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = lozzeg::timestamp_to_str_gm(tx_recieve_timestamp);
...
show_ringmembers_hex(string const& tx_hash_str)
{
    ...
    vector<txin_to_key> input_key_imgs = lozzeg::get_key_images(tx);
                ...
                bool tx_prove = false)
{
    ...
    if (lozz_address_str.empty())
    {
        return string("Lozzax address not provided!");
    }
    ...
    if (!lozzeg::parse_str_address(lozz_address_str,  address_info, nettype))
    {
        cerr << "Cant parse string address: " << lozz_address_str << endl;
        return string("Cant parse lozz address: " + lozz_address_str);
    }
    ...
    if (!lozzeg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!lozzeg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = lozzeg::timestamp_to_str_gm(blk.timestamp);
    ...
    mstch::map context {
            ...
            {"lozz_address"          , lozz_address_str},
            {"viewkey"              , viewkey_str_partial},
            {"tx_pub_key"           , pod_to_hex(txd.pk)},
            {"blk_height"           , tx_blk_height_str},
            {"tx_size"              , fmt::format("{:0.4f}",
                                                  static_cast<double>(txd.size) / 1024.0)},
            {"tx_fee"               , lozzeg::lozz_amount_to_str(txd.fee, "{:0.12f}", true)},
            {"blk_timestamp"        , blk_timestamp},
    ...
    string server_time_str = lozzeg::timestamp_to_str_gm(server_timestamp, "%F");
    ...
    for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some lozz.
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
        ...
        if (mine_output)
        {
            sum_lozz += outp.second;
        }

        outputs.push_back(mstch::map {
                {"out_pub_key"           , pod_to_hex(outp.first.key)},
                {"amount"                , lozzeg::lozz_amount_to_str(outp.second)},
    ...
    // to hold sum of lozz in matched mixins, those that
    // perfectly match mixin public key with outputs in mixn_tx.
    uint64_t sum_mixin_lozz {0};
    ...
    // we get number of all posibilities, and their total lozz amount
    ...
    for (const txin_to_key& in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", lozzeg::lozz_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& abs_offset: absolute_offsets)
        {
            ...
            public_key mixin_tx_pub_key
                    = lozzeg::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = lozzeg::get_ouputs_tuple(mixin_tx);
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , lozzeg::lozz_amount_to_str(amount)}
                });
                ...
                if (mine_output)
                {
                    ...
                    // increase sum_mixin_lozz only when
...
//
//                        if (!lozzeg::generate_key_image(derivation,
...
//                                << endl;
    ...
    context["found_our_outputs"] = (sum_lozz > 0);
    context["sum_lozz"]           = lozzeg::lozz_amount_to_str(sum_lozz);

    context.emplace("inputs", inputs);

    context["show_inputs"]   = show_key_images;
    context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
    context["sum_mixin_lozz"] = lozzeg::lozz_amount_to_str(
            sum_mixin_lozz, "{:0.12f}", false);
    ...
    // useful for unit testing as it provides total lozz sum
    // of possible mixins
    ...
    if (sum_mixin_lozz > (sum_lozz + txd.fee)
        && no_of_matched_mixins == inputs.size())
    {
        //                  (outcoming    - incoming) - fee
        possible_spending = (sum_mixin_lozz - sum_lozz) - txd.fee;
    }

    context["possible_spending"] = lozzeg::lozz_amount_to_str(
           ...
           string domain)
{
    ...
    return show_my_outputs(tx_hash_str, lozz_address_str,
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = lozzeg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , lozzeg::lozz_amount_to_str(tx_change.amount)},
                        {"has_payment_id"     , (payment_id  != null_hash)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id"         , pid_str},
                        {"payment_id8"        , pid8_str},
                ...
                for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , lozzeg::lozz_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , lozzeg::lozz_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   lozzeg::lozz_amount_to_str(sum_outputs_amounts)});
                ...
                tx_cd_data["min_mix_time"]     = lozzeg::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = lozzeg::timestamp_to_str_gm(max_mix_timestamp);
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            for (tx_destination_entry& a_dest: ptx.construction_data.splitted_dsts)
            {
                ...
                outputs_lozz_sum += a_dest.amount;
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        lozzeg::lozz_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_lozz_sum"] = lozzeg::lozz_amount_to_str(outputs_lozz_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = lozzeg::lozz_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
            {
                ...
                inputs_lozz_sum += tx_source.amount;
            }

            // mark that we have signed tx data for use in mstch
            tx_context["have_raw_tx"] = true;

            // provide total mount of inputs lozz
            tx_context["inputs_lozz_sum"] = lozzeg::lozz_amount_to_str(inputs_lozz_sum);
            ...
            for (mstch::node& input_node: inputs)
            {
                ...
                amount = lozzeg::lozz_amount_to_str(real_amounts.at(input_idx));
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!lozzeg::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = lozzeg::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = lozzeg::decrypt(
    ...
    // get lozz address stored in this key image file
    const account_public_address* lozz_address =
            reinterpret_cast<const account_public_address*>(
                    decoded_raw_data.data());

    address_parse_info address_info {*lozz_address, false};


    context.insert({"address"        , REMOVE_HASH_BRAKETS(
            lozzeg::print_address(address_info, nettype))});
    context.insert({"viewkey"        , REMOVE_HASH_BRAKETS(
            fmt::format("{:s}", prv_view_key))});
    context.insert({"has_total_lozz"  , false});
    context.insert({"total_lozz"      , string{}});
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*lozz_address, false, false, crypto::null_hash8};
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        uint64_t lozz_amount = td.amount();
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = lozzeg::get_tx_pub_key_from_received_outs(tx);
        ...
        mstch::map output_info {
                ...
                {"amount"              , lozzeg::lozz_amount_to_str(lozz_amount)},
                {"tx_hash"             , REMOVE_HASH_BRAKETS(fmt::format("{:s}", td.m_txid))},
                {"timestamp"           , lozzeg::timestamp_to_str_gm(blk_timestamp)},
    ...
    if (total_lozz > 0)
    {
        context["has_total_lozz"] = true;
        context["total_lozz"] = lozzeg::lozz_amount_to_str(total_lozz);
    }
...
search(string search_text)
{
    ...
    if (search_str_length == 95)
    {
        ...
        if (!lozzeg::parse_str_address(search_text, address_info, nettype_addr))
        {
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    string address_str      = lozzeg::print_address(address_info, nettype);
    ...
    mstch::map context {
            {"lozz_address"        , REMOVE_HASH_BRAKETS(address_str)},
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", lozzeg::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = lozzeg::timestamp_to_str_gm(tx_timestamp);
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_lozz_sum_not_zero");
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!lozzeg::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , lozzeg::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Lozzax address not provided";
        return j_response;
    }
    ...
    if (!lozzeg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_blk_no   = std::to_string(current_values.blk_no - 1);
        string emission_coinbase = lozz_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = lozz_amount_to_str(current_values.fee, "{:0.4f}", false);
...
get_tx_json(const transaction& tx, const tx_details& txd)
{
    ...
    json j_tx {
            ...
            {"mixin"       , txd.mixin_no},
            {"tx_size"     , txd.size},
            {"lozz_outputs" , txd.lozz_outputs},
            {"lozz_inputs"  , txd.lozz_inputs},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = LOZZ_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
            {"tx_fee"                , lozzeg::lozz_amount_to_str(txd.fee, "{:0.12f}", false)},
            {"tx_fee_micro"          , lozzeg::lozz_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , lozzeg::lozz_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  lozzeg::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = lozzeg::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = lozzeg::timestamp_to_str_gm(max_mix_timestamp);
    ...
    context["inputs_lozz_sum_not_zero"]  = (inputs_lozz_sum > 0);
    context["inputs_lozz_sum"]           = lozzeg::lozz_amount_to_str(inputs_lozz_sum);
    ...
    context["outputs_lozz_sum"] = lozzeg::lozz_amount_to_str(outputs_lozz_sum);
...
)
{
    ...
    {
        // get mixins in time scale for visual representation
        pair<string, double> mixin_times_scale = lozzeg::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    txd.pk = lozzeg::get_tx_pub_key_from_received_outs(tx);
    txd.additional_pks = cryptonote::get_additional_tx_pub_keys_from_extra(tx);


    // sum lozz in inputs and ouputs in the given tx
    const array<uint64_t, 4>& sum_data = summary_of_in_out_rct(
            tx, txd.output_pub_keys, txd.input_key_imgs);

    txd.lozz_outputs       = sum_data[0];
    txd.lozz_inputs        = sum_data[1];
...
get_footer()
{
    ...
    string footer_html = mstch::render(lozzeg::read(TMPL_FOOTER), footer_context);
