...
#ifndef LOZZEG01_TOOLS_H
#define LOZZEG01_TOOLS_H

#define PATH_SEPARARTOR '/'

#define LOZZ_AMOUNT(value) \
...
static
string
lozz_amount_to_str(const uint64_t& lozz_amount,
                  ...
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, LOZZ_AMOUNT(lozz_amount));
    }
    else
    {
        if (lozz_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, LOZZ_AMOUNT(lozz_amount));
        }
    }
