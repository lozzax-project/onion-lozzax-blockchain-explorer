# Onion Lozzax Blockchain Explorer

Currently available Lozzax blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Lozzax testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Lozzax Blockchain Explorer. The example not only shows how to use
Lozzax C++ libraries, but also demonstrates how to use:
...
Clearnet versions:
 - [https://lozzchain.net/](https://lozzchain.net/) - https enabled, most popular and very stable.
 - [https://monerohash.com/explorer/](https://monerohash.com/explorer/) - nice looking one, https enabled.
 - [http://monerochain.com/](http://monerochain.com/) - JSON API based, multiple nodes.   
 - [https://blox.minelozz.com/](https://blox.minelozz.com/) - - https enabled.
 - [https://community.lozz.to/explorer/mainnet/](https://community.lozz.to/explorer/mainnet/)

Testnet version:

 - [https://testnet.lozzchain.com/](https://testnet.lozzchain.com/) - https enabled.
 - [https://community.lozz.to/explorer/testnet/](https://community.lozz.to/explorer/testnet/)

Stagenet version:
 
 - [https://stagenet.lozzchain.net/](https://stagenet.lozzchain.net/)
 - [https://community.lozz.to/explorer/stagenet/](https://community.lozz.to/explorer/stagenet/)

i2p users (main Lozzax network):
...
## Onion Lozzax Blockchain Explorer features

The key features of the Onion Lozzax Blockchain Explorer are:
 ...
 - showing public components of Lozzax addresses,
 - decoding which outputs and mixins belong to the given Lozzax address and viewkey,
 - can prove that you send Lozzax to someone,
 - detailed information about ring members, such as their age, timescale, and ring sizes,
 - showing number of amount output indices,
 - support Lozzax testnet and stagnet networks,
...
#### Lozzax download and compilation

To download and compile recent Lozzax follow instructions
...
Once the Lozzax compiles, the explorer can be downloaded and compiled
as follows:
...
By default it will look for blockchain in its default location i.e., `~/.bitlozzax/lmdb`.
You can use `-b` option if its in different location.

For example:

```bash
./lozzblocks -b /home/mwo/non-default-monero-location/lmdb/
```

Example output:

```bash
[mwo@arch onion-monero-blockchain-explorer]$ ./lozzblocks
...
```
lozzblocks, Onion Lozzax Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Lozzax total emission monitoring
                                        thread
  ...
  --ssl-key-file arg                    path to key file for ssl (https)
                                        functionality
  -d [ --deamon-url ] arg (=http:://127.0.0.1:11112)
                                        Lozzax daemon url
...
alias lozzblocksmainnet='~/onion-monero-blockchain-explorer/build/lozzblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias lozzblockstestnet='~/onion-monero-blockchain-explorer/build/lozzblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

## Enable Lozzax emission

Obtaining current Lozzax emission amount is not straight forward. Thus, by default it is
disabled. To enable it use `--enable-emission-monitor` flag, e.g.,


```bash
lozzblocks --enable-emission-monitor
```
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.bitlozzax/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.bitlozzax/testnet/lmdb/emission_amount.txt` or `~/.bitlozzax/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.bitlozzax/lmdb/emission_amount.txt`
...
```
Lozzax emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Lozzax daemon's  `print_coinbase_tx_sum` command.
...
Having the `crt` and `key` files, run `lozzblocks` in the following way:

```bash
./lozzblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key
```
...
```bash
# we use here official Lozzax project's donation address as an example
...
We use recipient's address (i.e. not our address from which we sent lozz to recipient).
...
Return raw json block data, as represented in Lozzax.
...
Return raw json tx data, as represented in Lozzax.
